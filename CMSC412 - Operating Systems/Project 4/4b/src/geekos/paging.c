...
/*
 * Project Deets: Demand Paging
 * You will modify this page fault handler to determine an appropriate action to take based on the address.
 * In the case that the address is within one page of the current stack limit, you should allocate a new page, 
 * map it into the appropriate address, and then return normally from the handler.  
 * The program will now be able to use the memory that you just allocated for it.  
 * In order to test your new page fault handler, run the provided program rec.c.
 */
/*
...
/*static*/ void Page_Fault_Handler(struct Interrupt_State *state) {
    ...
    pte_t *currTable;
    int pdi, pti;
    uint_t roundedAddress;
    void *page;
    uint_t espAddr, roundEspAddr;
    //struct User_Interrupt_State *us = (struct User_Interrupt_State *) state;
    //espAddr = us->espUser + USER_VM_START;
    //roundEspAddr = Round_Up_To_Page(espAddr);
    ...
    currDir = g_currentThread->userContext->pageDir;
    roundedAddress = Round_Down_To_Page(address);
    pdi = PAGE_DIRECTORY_INDEX(roundedAddress);
    pti = PAGE_TABLE_INDEX(roundedAddress);


    /*Fault is within one page of the current stack limit */
    // if address + PAGE_SIZE <
    if(address >= g_currentThread->userContext->dataEnd){
    
      if(currDir[pdi].present != 0)
	currTable = (pte_t *) ((unsigned int)(currDir[pdi].pageTableBaseAddr) << 12);
      else{
	currTable = Alloc_Page();
	if(currTable == 0){
	  Print("Unexpected Page Fault received\n");
	  Print_Fault_Info(address, faultCode);
	  Dump_Interrupt_State(state);
	...
	currDir[pdi].present = 1;
	currDir[pdi].pageTableBaseAddr = (uint_t) PAGE_ALIGNED_ADDR(currTable);
	currDir[pdi].flags = VM_USER|VM_WRITE;
      }
      //grow new stack
      page = Alloc_Pageable_Page(&currTable[pti], USER_VM_START + pti * PAGE_SIZE);
      if(page == 0){
	Print("Unexpected Page Fault received\n");
	Print_Fault_Info(address, faultCode);
	Dump_Interrupt_State(state);
	Exit(-1);
      ...
      currTable[pti].present = 1;
      currTable[pti].flags = VM_USER|VM_WRITE;
      currTable[pti].pageBaseAddr = PAGE_ALIGNED_ADDR(page);
    }else{
      	Print("Unexpected Page Fault received\n");
	Print_Fault_Info(address, faultCode);
	Dump_Interrupt_State(state);
...
void Init_VM(struct Boot_Info *bootInfo) {
  ...
  d_entry.present = 1;
  d_entry.flags = VM_WRITE;
  
  
  t_entry.present = 1;
  t_entry.flags = VM_WRITE;
