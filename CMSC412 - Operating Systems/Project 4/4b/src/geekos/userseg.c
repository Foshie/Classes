...
static struct User_Context *Create_User_Context(ulong_t size) {
    ...
    Init_Code_Segment_Descriptor(&context->ldt[0],(ulong_t) USER_VM_START,Round_Up_To_Page((USER_VM_END - USER_VM_START) / PAGE_SIZE), USER_PRIVILEGE);
    Init_Data_Segment_Descriptor(&context->ldt[1], (ulong_t) USER_VM_START,Round_Up_To_Page((USER_VM_END - USER_VM_START) / PAGE_SIZE), USER_PRIVILEGE);
