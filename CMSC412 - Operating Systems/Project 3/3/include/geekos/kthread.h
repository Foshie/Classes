...
DEFINE_LIST(My_Thread_Queue, Kernel_Thread);
/*
...
#define MAX_TLOCAL_KEYS 128
  ...
  /* Determins if thread should be a detached process */
  int detached;
  /* Keeps track of thread's program name
   * Will be NULL for kernel threads
   */
#define PROG_NAME_LENGTH 128
  char name[PROG_NAME_LENGTH];
  
  //Project 3 Scheduler
  DEFINE_LINK(My_Thread_Queue, Kernel_Thread);
  int currRunQLvl;
	int countLowLvl;
};
...
IMPLEMENT_LIST(My_Thread_Queue, Kernel_Thread);
...
static __inline__ void Enqueue_My_Thread(struct My_Thread_Queue *queue,
                                      struct Kernel_Thread *kthread) {
    Add_To_Back_Of_My_Thread_Queue(queue, kthread);
}

static __inline__ void Remove_My_Thread(struct My_Thread_Queue *queue,
                                     struct Kernel_Thread *kthread) {
    Remove_From_My_Thread_Queue(queue, kthread);
}
