...
#define NUM_USER_LDT_ENTRIES 2
	...
	/*  Project 2 Edits
	 *	Signal Handler
	 *	sig_h stores the handler function to the corresponding signal num
	 */
	 signal_handler sig_h[MAXSIG];
	//flag array for signals to be handled
	int pending[MAXSIG];
	/*trampoline function*/
	void *tramp;

	 //Current Signal being handled
	 int curr_sig;

#if 1
	#define MAX_SEM_SIZE 20
	//Used to keep track of what semaphores are being called by user.  
    int semaphores[MAX_SEM_SIZE];
#endif
};
...
int Spawn(const char *program, const char *command,
          struct Kernel_Thread **pThread, int detached);
