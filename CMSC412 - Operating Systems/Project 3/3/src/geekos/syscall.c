...
static int Sys_GetKey(struct Interrupt_State *state) {
  //if current thread is detached or root is detached
  struct Kernel_Thread *root = g_currentThread;
  if(root->detached == 1){
...
static int Sys_Spawn(struct Interrupt_State *state) {
  ...
  done:    /*IF the process has defined a signal handler for this signal, this function will have to do the following:
		 int curr_sig;
		Push onto the user stack a snapshot of the interrupt state that is currently stored at the top of the kernel stack.  The interrupt state is the topmost portion of the kernel stack, defined in include/geekos/int.h in struct Interrupt_State, shown above.
		Push onto the user stack the number of the signal being delivered.
		Push onto the user stack the address of the "signal trampoline" that invokes the Sys_ReturnSignal system call, and was registered by the Sys_RegDeliver system call, mentioned above.
		Change the current kernel stack such that (notice that you already saved a copy in the user stack)
		(1) The user stack pointer is updated to reflect the changes made in step 3 - 5.
...
static int Sys_PS(struct Interrupt_State *state) {
  ...
  int length = state->ecx, ind = 0;
  struct Process_Info psTable[length];
  
  length = get_PS(psTable, length);
  if(length >= state->ecx)
    return 0;
  if(Copy_To_User(state->ebx, psTable, sizeof(psTable)));
  return length;
...
static int Sys_Kill(struct Interrupt_State *state) {
	//state->ecx dummy number for now.
	//send state->ebx into some Kill() in pthread
  int signal = state->ecx;
  /* Project 1
  return Kill(state->ebx, signal);
  */
...
static int Sys_Signal(struct Interrupt_State *state) {
  ...
  if(!IS_SIGNUM(state->ecx))
    return -1;
  to_return = Set_Handler(state->ebx, state->ecx, g_currentThread);
  if(to_return = -1)
    return EINVALID;
  return to_return;
}
...
static int Sys_RegDeliver(struct Interrupt_State *state) {
  g_currentThread->userContext->tramp = state->ebx;
    return 0;
}
...
static int Sys_WaitNoPID(struct Interrupt_State *state) {
  int pid;
  pid = wait_noPID(g_currentThread, state);
  if(pid != -1)
    return pid;
  return ENOZOMBIES;
}
