...
int Spawn(const char *program, const char *command,
    ...
    /* Start the process! */
    process = Start_User_Thread(userContext, detached);

    if (process != 0) {
      //Project 1 addition.  Unnamed threads signify kernel thread
     
    strcpy(process->name, program);
     
        KASSERT(process->refCount == 2 || process->refCount == 1);
