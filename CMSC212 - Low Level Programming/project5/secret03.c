 ...
 *
 * Creates an Array_hash_map with several elements, resizes it (first
...
int main() {
  ...
  char letters[NUM_ITEMS]= {'F', 'M', 'T', 'M', 'S', 'W', 'U', 'H', 'M', 'T',
                            'H', 'S', 'U', 'U', 'U', 'M', 'T', 'T', 'W', 'W',
                            'W', 'H'};
  int new_keys[4]= {5, 19, 23, 27};

  hash_map= create_example_hash_map(days, letters);

  if (size(hash_map) == 22 && current_capacity(hash_map) == 30) {
    ...
    for (i= 0; i < 3 && correct; i++)
      if (index_of(hash_map, (Key) (long) new_keys[i]) != i)
        correct= 0;

    /* check that some of the keys are present */
    if (size(hash_map) == 4 && current_capacity(hash_map) == 10 && correct)
