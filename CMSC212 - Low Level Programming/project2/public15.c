 ...
 * computation (computes the factorial of a number) and contains a function.
 * Checks the contents of registers afterward.
 */
...
int main() {
  ...
  short exp_registers[NUM_REGISTERS]= {0, 2, 10, 5040, 0, 0, 0xfe, 0x12};
  int num_instructions;
  Status ret;

  num_instructions= assemble("public15.a", mem);

  if (num_instructions == 0)
    printf("Regretfully the program couldn't be assembled.  Bye.\n");
  else {

    ret= execute_program(mem, registers, 0, 43, 0);
