 ...
 *
 * Tests execute_program() with jump instructions.  Checks the contents of
 * registers afterward, and also verifies the final values of the stack
 * pointer and progam counter registers.
 */
...
int main() {
  ...
  short exp_registers[NUM_REGISTERS]= {0, 212, 0, 214, 0, 216, 0xfe, 0x12};
  int num_instructions;
  Status ret;

  num_instructions= assemble("public08.a", mem);

  if (num_instructions == 0)
    printf("Regretfully the program couldn't be assembled.  Bye.\n");
  else {

    ret= execute_program(mem, registers, 0, 7, 0);
