 ...
 *
 * Calls execute_program() with a program that performs an actual
 * computation (computes the greatest common divisor of two numbers using
 * Euclid's algorithm).  Checks the contents of registers afterward.
 */
...
int main() {
  ...
  short exp_registers[NUM_REGISTERS]= {0, 16, 0, 16, 2, 10, 0xfe, 0x16};
  int num_instructions;
  Status ret;

  num_instructions= assemble("public14.a", mem);

  if (num_instructions == 0)
    printf("Regretfully the program couldn't be assembled.  Bye.\n");
  else {

    ret= execute_program(mem, registers, 0, 23, 0);
