...
/* Comparison according to Manhattan distance to origin */
static int cmp(const void * a, const void * b) {
  int dist1, dist2;
  point2D *first = (point2D *) a;
  point2D *second = (point2D *) b;

  dist1 = (first->x > 0 ? first->x : -first->x) + 
            (first->y > 0 ? first->y : -first->y);
  dist2 = (second->x > 0 ? second->x : -second->x) + 
              (second->y > 0 ? second->y : -second->y);
